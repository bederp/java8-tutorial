...
/**
 * 1) Create a map which prints amount of person of each age use stream.collect(Collectors.groupingBy(Function,
 * Collectors.mapping(Function, Collectors.counting()))) 2) Create a map Gender->String with gender as keys and
 * comma-separated names as values use stream.collect(Collectors.groupingBy(Function, Collectors.mapping(Function,
 * Collectors.joining(",")))) 3) Calculate and print the average age of females use
 * stream.filter(Predicate).mapToInt(Function).average() 4) Create a map with genders as keys and average ages as values
 * use stream.collect(Collectors.groupingBy(Function, Collectors.averagingInt(Function)) 5) Print the list of persons in
 * alphabetical order of names use stream.sorted(Comparator.comparing(Function)) 6) Print the list of persons in order
 * of gender, then name: use stream.sorted(Comparator.comparing(Function).thenComparing(Function)
 */
